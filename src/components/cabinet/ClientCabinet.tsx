...
export const ClientCabinet: React.FC = () => {
  ...
  const handleLogin = useCallback(async () => {
    if (!loginEmail.trim()) { addNotification('Indtast din email', 'info'); return; }
    setIsProcessing(true);
    const result = await loginByEmail(loginEmail.trim());
    if (result === 'found') {
      addNotification(`Velkommen tilbage!`, 'success');
      setAuthMode('idle');
      setLoginEmail('');
    } else {
      addNotification('Ingen profil fundet. Opret en ny?', 'error');
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!reg.first_name || !reg.email) { addNotification('Fornavn og email er påkrævet', 'error'); return; }
    setIsProcessing(true);
    try {
      ...
      addNotification(err.message || 'Fejl ved oprettelse', 'error');
    }
    setIsProcessing(false);
  };
  ...
  const statusLabel = (o: any) => {
    ...
    const map: Record<string, { label: string; cls: string }> = {
      in_transit:         { label: 'I transit',          cls: 'bg-emerald-50 text-emerald-600' },
      awaiting_transport: { label: 'Afventer transport', cls: 'bg-purple-50 text-purple-600' },
      in_progress:        { label: 'I arbejde',          cls: 'bg-amber-50 text-amber-600' },
      accepted:           { label: 'Modtaget',           cls: 'bg-blue-50 text-blue-600' },
      processing:         { label: 'Behandles',          cls: 'bg-blue-50 text-blue-500' },
      confirmed:          { label: 'Bekræftet',          cls: 'bg-emerald-50 text-emerald-600' },
      cancelled:          { label: 'Annulleret',         cls: 'bg-rose-50 text-rose-500' },
      delivered:          { label: 'Leveret',            cls: 'bg-slate-100 text-slate-500' },
    };
      ...
      <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-2xl space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">
            {authMode === 'idle' ? 'Min Konto' : authMode === 'login' ? 'Log ind' : 'Opret profil'}
          </h2>
          {authMode === 'idle' && (
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest max-w-xs mx-auto leading-relaxed">
              Log ind for at se din profil, ordrehistorik og gemte adresser
            </p>
          )}
        </div>
        ...
        {authMode === 'idle' ? (
          ...
          <div className="space-y-4">
            ...
            <p className="text-[9px] text-slate-400 text-center font-bold tracking-wide">
              Vi finder din profil baseret på din email
            </p>
          ...
          <form onSubmit={handleRegister} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['private', 'business'] as const).map(t => (
                <button key={t} type="button" onClick={() => setReg(r => ({ ...r, client_type: t }))}
                  className={`py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest border-2 transition-all ${reg.client_type === t ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-400 border-slate-100 hover:border-slate-300'}`}>
                  {t === 'private' ? '👤 Privat' : '🏢 Firma'}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { key: 'first_name', placeholder: 'Fornavn *', required: true },
                { key: 'last_name',  placeholder: 'Efternavn' },
              ].map(f => (
            ...
            <div className="grid grid-cols-3 gap-2">
              <input value={reg.street} onChange={e => setReg({ ...reg, street: e.target.value })}
                placeholder="Gade" className="col-span-2 bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-4 text-sm font-bold outline-none focus:border-emerald-400 transition-all" />
              <input value={reg.house_number} onChange={e => setReg({ ...reg, house_number: e.target.value })}
                placeholder="Nr." className="bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-4 text-sm font-bold outline-none focus:border-emerald-400 transition-all" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input value={reg.postal_code} onChange={e => setReg({ ...reg, postal_code: e.target.value })}
                placeholder="Postnr." className="bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-4 text-sm font-bold outline-none focus:border-emerald-400 transition-all" />
              <input value={reg.city} onChange={e => setReg({ ...reg, city: e.target.value })}
                placeholder="By" className="bg-slate-50 border-2 border-slate-100 rounded-2xl px-4 py-4 text-sm font-bold outline-none focus:border-emerald-400 transition-all" />
            </div>
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-start">
        ...
        <aside className="w-full lg:w-72 shrink-0 space-y-4">
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div>
                <h3 className="text-xl font-black uppercase tracking-tighter">
                  {currentUser ? currentUser.name : 'Gæst'}
                </h3>
                ...
                {currentUser?.discount && currentUser.discount > 0 && (
                  <div className="flex items-center justify-center gap-1 mt-2 bg-amber-500/10 text-amber-400 px-3 py-1 rounded-full text-[9px] font-black uppercase">
                    <Percent size={10} /> {currentUser.discount}% rabat
                  </div>
                )}
              </div>
              <div className="flex items-center gap-2 bg-emerald-500/10 text-emerald-400 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border border-emerald-500/20">
                <ShieldCheck size={11} /> {currentUser ? 'Verificeret' : 'Gæst session'}
              </div>
              ...
              {cartCount > 0 && (
                <button onClick={goToCheckout}
                  ...
                  <div className="flex items-center gap-2"><ShoppingCart size={16} /><span>Til kassen</span></div>
                ...
                <div className="w-full rounded-3xl bg-white/5 border border-white/10 p-4 space-y-3">
                  ...
                  {push.status === 'denied' && (
                    <p className="text-[9px] font-bold text-rose-400 leading-relaxed">
                      Adgang blokeret — tillad notifikationer i browserindstillingerne.
                    </p>
                  )}

                  {!import.meta.env.VITE_VAPID_PUBLIC_KEY && (
                    <p className="text-[9px] font-bold text-amber-400">
                      Push er ikke konfigureret endnu.
                    </p>
                  )}
                    ...
                    <>
                      <div className={`flex items-center gap-3 p-3 rounded-2xl border ${push.isSubscribed ? 'bg-emerald-900/30 border-emerald-700/50' : 'bg-white/5 border-white/10'}`}>
                        ...
                        <p className="text-[9px] font-bold text-slate-300 leading-relaxed">
                          {push.isSubscribed
                            ? 'Aktiv — du modtager opdateringer om dine ordrer'
                            : 'Modtag besked når din ordre opdateres'
                          }
                        </p>
                      </div>
                      ...
                      {push.isSubscribed ? (
                        ...
                        <button onClick={push.subscribe}
                          ...
                          <Bell size={12} /> Aktiver notifikationer
                        </button>
                      )}
                    </>
                  )}
          ...
          <nav className="bg-white rounded-[2.5rem] p-3 border border-slate-100 shadow-lg flex flex-col gap-1">
            {[
              ...
              { id: 'history', label: 'Ordrehistorik', icon: ShoppingBag },
            ].map(tab => (
        ...
        <main className="flex-1 w-full min-w-0">
            ...
            <div className="bg-white rounded-[3.5rem] p-12 border border-slate-100 shadow-2xl space-y-8">
              {currentUser ? (
                <>
                  ...
                  <div className="flex items-center gap-4 pb-6 border-b border-slate-50 flex-wrap">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                        {currentUser.client_type === 'business' ? 'Erhvervskunde' : 'Privatkunde'} · Verificeret
                      </p>
                    </div>
                  </div>
                  ...
                  {currentUser.client_type === 'business' && currentUser.company_name && (
                    <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100 flex items-start gap-4">
                      ...
                      <div>
                        <div className="text-[8px] font-black text-blue-400 uppercase tracking-widest mb-1">Virksomhed</div>
                  ...
                  {(fullAddr || cityLine) && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="p-5 bg-slate-50 rounded-2xl border border-slate-100 flex items-start gap-3">
                        ...
                        <div>
                          <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Levering</div>
                          {currentUser.delivery_same_as_billing ? (
                            <div className="text-xs text-slate-400 italic">Samme som fakturering</div>
                            ...
                            <div className="text-xs text-slate-400 italic">Ikke angivet</div>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  {currentUser.discount && currentUser.discount > 0 ? (
                    <div className="flex items-center gap-3 p-5 bg-amber-50 rounded-2xl border border-amber-100">
                      ...
                      <div>
                        <div className="text-[8px] font-black text-amber-400 uppercase tracking-widest">Din personlige rabat</div>
                  ...
                  <button onClick={goToCatalog}
                    ...
                    <Zap size={14} /> Se produkter <ArrowRight size={14} />
                  </button>
                </>
              ) : (
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-6">
                  ...
                  <div>
                    <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter">Min konto</h2>
                      ...
                      <div key={item.id} className="flex items-center justify-between bg-slate-50 rounded-2xl p-5 gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Antal: {item.quantity}</div>
                          </div>
                        </div>
          ...
          {activeTab === 'history' && (
            <div className="bg-white rounded-[3.5rem] p-10 border border-slate-100 shadow-2xl space-y-6">
              <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter flex items-center gap-3">
                <ShoppingBag size={24} className="text-emerald-500" /> Ordrehistorik
              </h2>
              ...
              {!currentUser ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                  ...
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Ingen ordrer endnu</p>
                  <button onClick={goToCatalog}
                    className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase text-[9px] tracking-widest hover:bg-emerald-500 transition-all flex items-center gap-2">
                      ...
                      <div key={order.id} className="bg-slate-50 rounded-3xl p-6 border border-slate-100 space-y-4">
                        <div className="flex items-start justify-between gap-4 flex-wrap">
                          <div className="space-y-1">
                            <div className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Ordre</div>
                          ...
                          <div className="flex gap-4 flex-wrap pt-1">
                            {order.shipping_date && (
                              <div className="text-[9px] font-bold text-slate-400 flex items-center gap-1">
                                📤 Afsendt: {new Date(order.shipping_date).toLocaleDateString('da-DK')}
                              </div>
                            )}
                            {order.arrival_date && (
                              <div className="text-[9px] font-bold text-slate-400 flex items-center gap-1">
                                📥 Forv. ankomst: {new Date(order.arrival_date).toLocaleDateString('da-DK')}
                              </div>
                            )}
                          </div>
                        )}
                        ...
                        {oItems.length > 0 && (
                          <div className="space-y-1 pt-2 border-t border-slate-200">
                            ...
                            {oItems.length > 3 && (
                              <div className="text-[9px] font-bold text-slate-400">+{oItems.length - 3} varer til</div>
                            )}
                          </div>
                        )}
                      </div>

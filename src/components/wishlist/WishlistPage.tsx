...
// ── Відокремлений компонент модального вікна для запобігання втрати фокусу ──
const GuestBookingModal = memo(({ 
  product, 
  onClose, 
  onSubmit 
}: { 
  ...
  return (
    <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4"
      ...
      onClick={onClose}>
      <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl space-y-5" 
        ...
        <div className="bg-slate-50 rounded-2xl p-3 flex items-center gap-3">
          <img src={product.image || IMAGE_FALLBACK} alt="" className="w-10 h-10 object-contain rounded-xl bg-white p-1" />
          <div className="min-w-0">
            ...
            <p className="text-[9px] text-emerald-600 font-bold">{product.category}</p>
          </div>
        </div>
        <div className="space-y-2">
          ...
          <input type="email" placeholder="your@email.com *" value={email}
            ...
            onKeyDown={e => e.key === 'Enter' && onSubmit(name, email)}
            autoFocus
            className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 py-3.5 text-xs font-bold focus:outline-none focus:border-emerald-400 transition-all" />
        </div>
        ...
        <div className="flex-1 min-w-0 space-y-1.5">
          ...
          {isActive && (
            <div className={`flex items-center gap-1.5 text-[9px] font-bold ${countdown.expired ? 'text-rose-500' : 'text-slate-400'}`}>
              <Clock size={11} />
              {countdown.expired ? 'Expired' : `Expires in ${countdown.h}h ${countdown.m}m`}
            </div>
          )}
...
export const WishlistPage: React.FC = () => {
  ...
  const handleBookProduct = useCallback(async (p: Product) => {
    ...
    if (email && email.includes('@')) {
      ...
      await toggleWishlist(p, email, currentUser?.name || '');
    } else {
      setBookingModal(p);
    }
  }, [isInWishlist, bookings, currentUser, emailFilter, cancelBooking, setPendingEmail, toggleWishlist]);

  const handleBookingSubmit = useCallback(async (name: string, email: string) => {
    if (!bookingModal || !email.includes('@')) {
      addNotification(t('err_email_invalid'), 'error');
      return;
    }
    setPendingEmail(email);
    setEmailFilter(email);
    await toggleWishlist(bookingModal, email, name);
    setBookingModal(null);
  }, [bookingModal, addNotification, t, setPendingEmail, toggleWishlist]);
            ...
            {allCategories.map(cat => (
              <button key={cat} onClick={() => setCatFilter(cat)}
                className={`px-4 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                  catFilter === cat ? 'bg-slate-900 text-white shadow-md' : 'bg-white border border-slate-200 text-slate-400 hover:border-emerald-300 hover:text-emerald-600'
                }`}>
                {cat}
              </button>
            ))}
            ...
            return (
              <div key={p.id} className={`group bg-white border-2 rounded-3xl p-4 hover:shadow-lg transition-all flex flex-col ${booked ? 'border-rose-200 bg-rose-50' : 'border-slate-100 hover:border-emerald-300'}`}>
                <div className="relative aspect-square rounded-2xl overflow-hidden mb-3 bg-slate-50 border border-slate-50">
                  <img src={p.image || IMAGE_FALLBACK} alt={name} className="w-full h-full object-contain p-2 group-hover:scale-105 transition-transform duration-500" />

...
import { ShoppingCart, LayoutGrid, Calculator, Heart, User, Search, Scale, X, RefreshCw, Info, MapPin, Phone, Mail, Lock } from 'lucide-react';
      ...
      <header className="sticky top-0 z-[100] bg-white/80 backdrop-blur-xl border-b border-slate-100 px-6">
        <div className="container mx-auto h-24 flex items-center justify-between gap-4">
          <div className="flex items-center gap-6 shrink-0">
            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setView(AppView.CATALOG)}>
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="hidden md:flex items-center relative">
              <div className={`flex items-center bg-slate-100 rounded-2xl transition-all duration-300 ease-in-out overflow-hidden ${isSearchVisible ? 'w-48 pr-2' : 'w-12'}`}>
                ...
                <input type="text" placeholder={t('search_placeholder')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`bg-transparent border-none py-3 outline-none text-[10px] font-black uppercase tracking-widest w-full transition-opacity duration-200 ${isSearchVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} />
              </div>
            </div>

            <button onClick={() => setView(AppView.CABINET)} className={`p-4 rounded-2xl transition-all shadow-xl active:scale-90 flex items-center justify-center ${currentView === AppView.CABINET ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white hover:bg-slate-800'}`} title={t('nav_cabinet')}>
              <User size={22} />
            </button>

            <button onClick={() => setView(AppView.CART)} className={`relative p-4 rounded-2xl transition-all shadow-xl active:scale-90 flex items-center justify-center ${currentView === AppView.CART ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white hover:bg-slate-800'}`}>
              <ShoppingCart size={22} />
              {totalItems > 0 && <span className="absolute -top-1 -right-1 bg-emerald-400 text-white text-[10px] font-black w-6 h-6 flex items-center justify-center rounded-full border-4 border-white animate-bounce-short">{totalItems}</span>}
            </button>

            <div className="hidden xl:flex flex-col gap-1 bg-slate-50 p-1 rounded-xl border border-slate-100 shrink-0">
              {languages.map(lang => (
                <button key={lang.code} onClick={() => setLanguage(lang.code)} className={`px-2 py-0.5 rounded-lg text-[8px] font-black transition-all ${language === lang.code ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
                  {lang.label}
                </button>
              ))}
              <button onClick={() => refreshRates()} className={`p-1 rounded-lg hover:bg-white transition-all mx-auto ${isLoadingRates ? 'animate-spin text-emerald-500' : 'text-slate-300'}`} title="Refresh Rates"><RefreshCw size={10} /></button>
            </div>
          </div>
        </div>
      </header>
